...
/* globals */
char *arg_input_file, *arg_argument_file, *arg_output_file;
...
typedef enum {
  false,true} bool;
typedef enum {
  alphanumeric, numeric, delimiter, endfile} type_of_token;
typedef enum {
  is_symbol, is_number, is_cons} celltype;
typedef enum {
  left, right} backtype;
typedef enum {
  parsing, execing} gc_flag;
...
#define MAX_CONS 			16384	/* Cons Cells Available */
...
void dumpregisters(void)
{
  int	ch;
  if (dumpstack == true) {
    printf("s:"); putoutput(s);
  }
  if (dumpenvironment == true)	{
    printf("e:"); putoutput(e);
  }
  if (dumpcontrol == true) {
    printf("c:"); putoutput(c);
  }
  if (dumpdump == true) {
    printf("d:"); putoutput(d);
  }
...
void putexp(long exp)
{
  ...
  if (is[exp] == is_cons && marked[exp] == true)
    {
      printf(" [C]");
    }
  else
...
void gettoken(void)
{
  ...
  token[0] = '\0';
  do {
    ch = get_char_from_inbuffer();} while (isspace(ch));	
  if (ch == END_OF_INPUT) 
    {
      tokentype = endfile;
      token[0] = ')';
...
void getargfile(void)
{
  ...
  do gets(filename); while (strlen(filename) == 0);
#else
  char* filename = arg_argument_file;
#endif
...
{
  if (argc != 4) {
    printf("usage: %s <lkc-file> <argument-file> <output-file>\n", argv[0]);
    exit(1);
  }
  arg_input_file = argv[1];
  arg_argument_file = argv[2];
  arg_output_file = argv[3];
  allocate_arrays();
  initialize();
          ...
          while (keep_going == true)
            {
              ...
              if (keep_going == true)
                {
                  args = getinput(false);
                  setoutfile(); 	/*where is the output going? */
#if 0
