% SECD verification                                                 %
%                                                                   %
% FILE:         correctness_ADD.ml                                  %
%                                                                   %
% DESCRIPTION:  The correspondence between the specification for    % 
%               ADD instruction, and the computational effect of    %
%               the machine executing an ADD instruction.           %
%                                                                   %
...
% ================================================================= %
% Correctness goal for the ADD instruction.                         %
% ================================================================= %
let correctness_ADD = TAC_PROOF
(([
 ...
 "((s                  when (is_major_state mpc))(SUC t),    
    ...
    ADD_trans ((s                  when (is_major_state mpc))t,
 ...
 THEN IMP_RES_TAC ADD_opcode_lemma 		% opcode_bits assumption %
 THEN port[when]
 THEN in_conv_tac BETA_CONV
					% Next assumption %
 THEN IMP_RES_THEN (ASSUME_TAC o UNDISCH) ADD_Next'
 					% rewrite Timeof ... (SUC t) %
 THEN IMP_RES_n_THEN 2 port1 TimeOf_Next_lemma
 THEN port[o_THM]

					% abbreviate times for readability ... %
 THEN ABBREV_TAC "(TimeOf(is_major_state mpc)t)" "t':^ctime"
 ...
 THEN port[ADD_trans]                    % unfold spec %
% ================================================================= %
			                % expand the M_* ops %
 ...
 THEN port[LET_THM]
 THEN BETA_TAC
					% rewrite the M_Add operation %
 THEN IMP_RES_n_THEN 3 port1 (hd(IMP_CANON M_Add_unfold_lemma))
 ...
 THEN REPEAT CONJ_TAC
					% simplify _imp view %
 THEN IMP_RES_THEN (SUBST1_TAC o UNDISCH) ADD_state'
 THEN (REFL_TAC
...
% ================================================================= %
% ================================================================= %
save_thm(`correctness_ADD`,correctness_ADD);;
