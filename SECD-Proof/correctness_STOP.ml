% SECD verification                                                 %
%                                                                   %
% FILE:         correctness_STOP.ml                                 %
%                                                                   %
% DESCRIPTION:  The correspondence between the specification for    % 
%               STOP instruction, and the computational effect of   %
%               the machine executing an STOP instruction.          %
%                                                                   %
...
load_definition `top_SECD` `STOP_trans_relation`;;
% ================================================================= %
let mtime = ":num"
and ctime = ":num";;
...
% ================================================================= %
% Correctness goal for the STOP instruction.                        %
% ================================================================= %
let correctness_STOP = TAC_PROOF
(([
   % ----------------------------- (cdr c) is NIL %
 ...
 THEN IMP_RES_TAC STOP_opcode_lemma 		% opcode_bits assumption %
 THEN port[when]
 THEN in_conv_tac BETA_CONV
					% Next assumption %
 THEN IMP_RES_THEN (ASSUME_TAC o UNDISCH) STOP_Next'
 ...
 THEN port[STOP_trans_relation]         % unfold spec %
% ================================================================= %
			                % expand the M_* ops %
 THEN IMP_RES_THEN port1 car_cdr_mem_abs_lemma
 THEN port[LET_THM]
					% simplify _imp view %
 THEN IMP_RES_THEN (STRIP_ASSUME_TAC o UNDISCH) STOP_state'
 THEN art[state_abs_thm]
% ================================================================= %
					% only memory remains %
 THEN GEN_TAC
 THEN (FIRST_ASSUM (\th. is_forall (concl th) => (ASSUME_TAC(SPEC "a:word14" th))
		                               | NO_TAC))
 THEN COND_CASES_THEN (SUBST_ALL_TAC)
 THEN RULE_ASSUM_TAC (porr[COND_CLAUSES])
 THENL
 [ GEN_TAC
   THEN port[M_is_cons; M_Cdr]
   THEN port[mem_abs]
   THEN port[REP_ABS_Mem_Range_Abs]
   THEN port[o_THM]
   THEN port[Mem_Range_Abs]
   THEN port[SND]
   THEN REPEAT (COND_CASES_TAC THEN art[ISL; OUTL])
 ; port[mem_abs]
   THEN port[REP_ABS_Mem_Range_Abs]
   THEN port[o_THM]
   THEN art[]
...
% ================================================================= %
save_thm(`correctness_STOP`,correctness_STOP);;
