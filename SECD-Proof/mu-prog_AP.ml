...
let head =
 S (pair o CONJUNCT1)
   CONJUNCT2 
   ((uncurry reformat)
             (stepper f (theorem `mu-prog_init_proofs` `toc_AP_state`,
                         theorem `mu-prog_init_proofs` `toc_AP_nonmajor`)));;
...
let mid1 = uncurry reformat
 (stepper f
  (CONJ (ASSUME "(mpc (t:^mtime)) = ^(mk_word9_from_num 113)")
        (REFL "T"),
   snd_thm))
 ;;
...
let mid2 = uncurry reformat
 (stepper f
  (CONJ (ASSUME "(mpc (t:^mtime)) = ^(mk_word9_from_num 115)")
        (REFL "T"),
   snd_thm))
 ;;

let mid3 = uncurry reformat
 (stepper f
  (CONJ (ASSUME "(mpc (t:^mtime)) = ^(mk_word9_from_num 120)")
        (REFL "T"),
   snd_thm))
 ;;
...
let tail = uncurry reformat 
 (stepper f
  (CONJ (ASSUME "(mpc (t:^mtime)) = ^(mk_word9_from_num 130)")
        (REFL "T"),
   snd_thm))
 ;;
...
let (AP_st, AP_non) =
sequencer head
 [ consx1x2; mid1; consx1x2; mid2; consx1x2; mid3; consx1x2; tail ];;

let time = (snd o dest_comb o fst o dest_eq o concl o CONJUNCT1) AP_st;;

let ADD_0 = CONJUNCT1 ADD_CLAUSES
and ADD_SUC = (CONJUNCT1 (CONJUNCT2 (CONJUNCT2 ADD_CLAUSES)));;

let num_inst_simp = TAC_PROOF
(([], "8+(4+(2+(4+(5+(4+(10+(4+7))))))) = 48"),
 CONV_TAC
 (REPEATC
  (CHANGED_CONV
...
let AP_state,AP_Next =
 let thm1,thm2 = (SUBS[num_simp]AP_st,
                  SUBS[num_simp]AP_non)
 in
