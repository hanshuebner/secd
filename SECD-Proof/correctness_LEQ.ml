% SECD verification                                                 %
%                                                                   %
% FILE:         correctness_LEQ.ml                                  %
%                                                                   %
% DESCRIPTION:  The correspondence between the specification for    % 
%               LEQ instruction, and the computational effect of    %
%               the machine executing an LEQ instruction.           %
%                                                                   %
...
% ================================================================= %
% Correctness goal for the LEQ instruction.                         %
% ================================================================= %
let correctness_LEQ = TAC_PROOF
(([
 ...
 "((s                  when (is_major_state mpc))(SUC t),    
    ...
    LEQ_trans ((s                  when (is_major_state mpc))t,
 ...
 THEN IMP_RES_TAC LEQ_opcode_lemma 		% opcode_bits assumption %
 THEN port[when]
 ...
 THEN port[LEQ_trans]                    % unfold spec %
% ================================================================= %
			                % expand the M_* ops %

 THEN port[M_CDR]	                % rewrite M_CDR s  %
 THEN IMP_RES_THEN port1 car_cdr_mem_abs_lemma
 THEN port[M_CDR]	                % rewrite M_CDR (cdr s), etc.  %
 THEN IMP_RES_THEN port1 car_cdr_mem_abs_lemma

 THEN port[LET_THM]			% unwind the first 2 lets %
 THEN port[LET_THM]
 THEN BETA_TAC
					% rewrite the M_Leq %
 THEN IMP_RES_n_THEN 2 port1 (hd(IMP_CANON M_Leq_unfold_lemma))
 ...
 THEN IMP_RES_THEN ((\th. ASSUME_TAC th
		          THEN UNDISCH_TAC (concl th))o UNDISCH) LEQ_state'
 THEN IMP_RES_THEN ((\th. ASSUME_TAC th
		          THEN UNDISCH_TAC (concl th))o UNDISCH) LEQ_Next'
...
% ================================================================= %
% ================================================================= %
save_thm(`correctness_LEQ`,correctness_LEQ);;
